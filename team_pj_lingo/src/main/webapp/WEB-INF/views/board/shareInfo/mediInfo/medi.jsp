...
<script type="text/javascript">
	...
	$(document).ready(function() {
	    $('#findMedi').click(function() {
	        $.ajax({
	            url: '${path}/findMedi.hl',
	            type: 'GET',
	            dataType: 'html',
	            beforeSend: function() {
	                // 로딩 표시
	...
	$(document).ready(function() {
	    $('#findForeignerService').click(function() {
	        $.ajax({
	            url: '${path}/findForeignerService.hl',
	            type: 'GET',
	            dataType: 'html',
	            beforeSend: function() {
	                // 로딩 표시
	...
	$(document).ready(function() {
	    $('#findInsurance').click(function() {
	        $.ajax({
	            url: '${path}/findInsurance.hl',
	            type: 'GET',
	            dataType: 'html',
	            beforeSend: function() {
	                // 로딩 표시
		...
		<div id="container">
			...
			<!-- 왼쪽메뉴 끝 -->
				...
				<div id="section2">
					<div class="container">
	                  <div class="menu">
	                     <a id="findMedi">병원이용</a>
	                     <a id="findInsurance">건강보험</a> 
	                     <a id="findForeignerService">주요도시 국제진료센터</a> 
	                  </div>
						...
						<div id="s2_inner">
							...
							<div class="join">
								...
								<form name="loginform" action="loginAction.do" method="post" >
									...
									<div class="content">
										<br>
										<strong>병원은 종합병원과 전문 의원으로 나뉜다. 종합병원은 첨단 의료시설을 갖추고 다양한 진료과목과 많은 전공의사 및 보조 시설이 있지만, 
										환자가 많은 편이고 대기시간이 길은 편이다. 반대로 전문의원은 일반적으로 1~2명의 의사가 내과나 소아과처럼 특정한 진료과목으로 환자에게 진료서비스를 한다. 
										전문의원은 가까운 동네에 많이 있으며 대기시간이 짧아 감기 등 간단한 질병의 치료에 편리하다.</strong>
										<br>
										<br>
										<hr>
										<br>
								       <h5><i class="fas fa-caret-right"></i> 병원 예약 방법</h5>
										<ul>
											<li>신속하고 편리하게 병원을 이용하기 위해서는 사전 예약이 필요하다. 사전 예약은 병원에 따라 방문 또는 전화, 인터넷 홈페이지 등을 통해 가능하다.</li>
										</ul>
										<br>
										<h5><i class="fas fa-caret-right"></i> 의료 부담</h5>
										<ul>
											<li>
											대부분의 병원에서 신용카드로 납부 가능하며 원무과에서 처리한다. 간혹 국제진료소가 따로 분리된 병원에서는 국제진료소에서 따로 납부 가능하다. 
											일반적으로 대부분 종합병원에서는 요청 시 영문 영수증 발급이 가능하다. 보험 가입여부에 따라 진료비가 다르고, 
